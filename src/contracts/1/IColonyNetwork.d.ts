...
export class IColonyNetwork extends Contract {
  ...
  functions: {
    ...
    /**
     * Note that strictly, _nUpdates and _nPreviousUpdates don't need to be set - they're only used during dispute resolution, which these replacement log entries are never used for. However, for ease of resyncing the client, I have decided to include them for now.
    ...
    /**
     * colonyAddress will always be set if the replacement exists
     ...
     * @returns (address, int256, uint256, address, uint256, uint256) An object with the details of the log entry (if it exists)
     */
     ...
     * @param _tokenAddress Address of an ERC20 token to serve as the colony token Additionally token can optionally support `mint` as defined in `ERC20Extended` Support for `mint` in mandatory only for the Meta Colony Token
     ...
     * @returns rootHash bytes32 The current Reputation Root Hash
     */
     ...
     * @returns feeInverse The inverse of the network fee
     */
  ...
  estimate: {
    ...
    initialise(_resolver: string): Promise<BigNumber>;
