...
/** Additional options for the {@link ColonyEventManager} */
...
export class ColonyEventManager {
  ...
  /**
   * Create a {@link ColonyFilter} that keeps track of its event source
   *
   * To create a {@link ColonyFilter}, we need to not only give it the topics and addresses as required by _ethers_
   ...
   *
   * @example
   * Filter for all `DomainAdded` events between block 21830000 and 21840000 (across all deployed {@link ColonyNetwork} contracts)
   * ```typescript
   *  const domainAdded = colonyEvents.createFilter(
   *    colonyEventSource,
   *    'DomainAdded(address,uint256)',
