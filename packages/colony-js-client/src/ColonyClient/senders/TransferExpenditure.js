...
> {
  async owner(expenditureId: number) {
    ...
    // Get the expenditure
    const { owner } = await this.client.getExpenditure.call({ expenditureId });
  ...
  async send(inputValues: *, options: *) {
    ...
    if (isOwner) {
      return this.client.transferExpenditureViaOwnership.send(
        inputValues,
        options,
      );
    }
    return this.client.transferExpenditureViaArbitration.send(
      inputValues,
      options,
    );
  }
