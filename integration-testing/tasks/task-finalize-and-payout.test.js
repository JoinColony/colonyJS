...
} from '../../packages/colony-js-client';
...
test("Claim a task's payout", async t => {
  /*
  ...
  /*
     ...
     *
     * This is necessary since ava might run this test before the tokens one, in which case this won't
  ...
  t.true(moveFundsToPotTransaction.successful);
  /*
  ...
  t.is(workerPayoutValue.toNumber(), payoutValue);
  /*
  ...
  t.true(finalizeTaskTransaction.successful);
  const {
    status: finalizedTaskStatus,
  } = await managerColonyClient.getTask.call({
    taskId: newTaskId,
  });
  t.is(finalizedTaskStatus, FINALIZED_TASK_STATUS);
  /*
  ...
  t.true(claimWorkerPayoutTransaction.successful);
});
